...
if __name__ == '__main__':
    ...
    parser.add_argument('--csv_name', type=str, default='', help='the file name with the csv file')
    parser.add_argument('--output_file', type=str, default='train.record', help='output file name')
    args = parser.parse_args()
    csvfile = os.getcwd() + os.path.join(args.path,args.csv_name)
    ...
    PATH_TO_PRETRAIN_LABELS = 'data/mscoco_label_map.pbtxt'
    NUM_CLASSES = 90
        ...
        with tf.Session(graph=detection_graph) as sess:
            ...
            for i in range(len(imagefiles)):
                if newlabels[i] < 3:     # avoid to clasify unknown
                    ...
                    classes = np.squeeze(classes).astype(np.int32)# Use the trained record in order to generate the new set of data to train
                    boxes = np.squeeze(boxes)
                    scores = np.squeeze(scores)
                    for j in range(len(classes)):
                        if classes[j] == TRAFFIC_LIGHT_CLASS: #get the boxes only from Traffic lights
